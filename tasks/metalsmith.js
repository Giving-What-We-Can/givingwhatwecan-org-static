...
function build(buildCount) {
  ...
  colophonemes
    ...
    .use(
      logMessage(
        "BUILD TIME: " + moment().format("YYYY-MM-DD @ H:m"),
        chalk.dim,
        true
      )
    )
    ...
    .destination("../dest")
    .use(ignore(["**/.DS_Store", "styles/partials/**"]))
    ...
    .use(function (files, metalsmith, done) {
      ...
      meta.stats.numberPledgers = meta.stats.PledgersActive.length;
      meta.stats.numberCountries = meta.stats.TotalGWWCMemberCountries;
      meta.stats.amountPledged =
        meta.stats.ProjectedDonations.projectedDonationsTotal;
      done();
    })
    .use(function (files, metalsmith, done) {
      ...
      function apply(file, cb) {
        ...
        if (
          ...
          meta.contentful.content_type === "5TB8GVLD7UUMkEYSEuUGsM"
        ) {
    ...
    .use(function (files, metalsmith, done) {
      ...
      each(
        Object.keys(files).filter(minimatch.filter("**/*.html")),
        function (file, callback) {
          ...
          each(
            ...
            function () {
              ...
              meta.pageTitle = [pageTitleBase, meta.siteInfo.title]
                .filter((a) => a)
                .join(" · ");
              meta.description = meta.description || meta.excerpt;
              // TODO caveats
              if (meta.recommendationLevel) {
                meta.recommendationLevel = parseInt(
                  meta.recommendationLevel.split(" ")[0]
                );
              }
              if (meta.priority) {
    ...
    .use(function (files, metalsmith, done) {
      ...
      each(
        Object.keys(files).filter(
          minimatch.filter("@(content-block)/**/*.html")
        ),
        function (file, cb) {
          var meta = files[file];
    ...
    .use(function (files, metalsmith, done) {
      ...
      var menus = {};
      each(
        Object.keys(files).filter(minimatch.filter("@(menu)/**/*.html")),
        apply,
        function () {
          ...
          const givingRoutes = [
            ...
            { label: "Pledge", href: GWWC_PLEDGE_URL },
            {
    ...
    .use(function (files, metalsmith, done) {
      ...
      Object.keys(files)
        .filter(minimatch.filter("@(alert)/**/*.html"))
        .forEach(function (file) {
          ...
          if (
            ...
            (alert.endDate &&
              moment(alert.endDate, ISO_SHORT_DATE).isBefore(moment(), "day"))
          ) {
            delete files[file];
          }
        });
      ...
      collectionSlugs.forEach(function (slug) {
        ...
        if (page) {
          fields.forEach(function (field) {
            if (page[field]) {
              collectionData[slug][field === "contents" ? "_contents" : field] =
                page[field];
            }
          });
          delete files[file];
    ...
    .use(function (files, metalsmith, done) {
      ...
      Object.keys(files)
        .filter(minimatch.filter("@(" + collectionSlugs.join("|") + ")/**"))
        .forEach(function (file) {
          files[file].contents = new Buffer(
            files[file]._contents ? files[file]._contents : ""
          );
    ...
    .use(function (files, metalsmith, done) {
      ...
      var metadata = metalsmith.metadata();
      ["collections", "posts", "causes", "charities", "reports"].forEach(
        function (m) {
          delete metadata[m];
        }
      );
    ...
    .use(function (files, metalsmith, done) {
      ...
      each(
        Object.keys(files).filter(minimatch.filter("@(page)/**/*.html")),
        apply,
        done
      );
    ...
    .use(function (files, metalsmith, done) {
      ...
      function apply(file, cb) {
        files[file].breadcrumbs = files[file].parent
          ? getParent(files[file].parent, [files[file].slug])
          : [files[file].slug];
        files[file].path =
          files[file].slug !== "/" ? files[file].breadcrumbs.join("/") : ".";
        cb();
      }
      function getParent(parent, path) {
        ...
        if (!parent.fields)
          throw new Error(
            "Parent of " + path + "has no fields!" + parent.sys.id
          );
        if (!parent.fields.slug)
          throw new Error("Parent has no slug!" + parent.sys.id);
    ...
    .use(function (files, metalsmith, done) {
      ...
      each(
        Object.keys(files).filter(
          minimatch.filter("@(quotation|person|cause|report|charity)/**/*.html")
        ),
        apply,
        done
      );
    ...
    .use(function (files, metalsmith, done) {
      ...
      each(
        Object.keys(files).filter(minimatch.filter("@(post)/**/*.html")),
        apply,
        done
      );
      function apply(file, cb) {
      ...
      function apply(file, cb) {
        ...
        if (filepath) {
          ...
          if (filepath + "/index.html" !== file) {
            files[path.join(filepath !== "." ? filepath : "", "index.html")] =
              files[file];
            delete files[file];
          }
        }
    ...
    .use(function (files, metalsmith, done) {
      ...
      function parse(file, callback) {
        files[file].contents = parseHTML(
          files[file].contents.toString(),
          files[file],
          metalsmith.metadata().redirects
        );
        callback();
      }
    })
    ...
    .use(
      feed({
        ...
        image_url:
          "https://www.givingwhatwecan.org/images/favicons/apple-touch-icon-144x144.png",
        preprocess: (file) => ({
          ...file,
          author: file.author
            ? file.author.map((a) => a.fields.title).join(", ")
            : "",
        }),
    ...
    .use(function (files, metalsmith, done) {
      ...
      Object.keys(files)
        ...
        .forEach(function (file) {
          files[file].contents = sanitiseSwigTags(
            files[file].contents.toString()
          );
        });
      done();
    ...
    .use(function (files, metalsmith, done) {
      ...
      Object.keys(files).forEach((file) => {
        ...
        if (
          !meta.template &&
          meta.layout &&
          !minimatch(file, "@(content-block|quotation|alert)/**")
        ) {
    ...
    .use(function (files, metalsmith, done) {
      ...
      each(
        Object.keys(files).filter(minimatch.filter("**/index.html")),
        create,
        function (err) {
          if (err) throw err;
          done();
  ...
  colophonemes
    ...
    .use(function (files, metalsmith, done) {
      ...
      files["styles/app.min.css"] = {
        contents: files["styles/app.concat.min.css"].contents,
      };
